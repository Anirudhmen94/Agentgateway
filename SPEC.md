...
2. **Deterministic policy.** Unknown tools, tools outside `allowed_tools`, per-minute rate limits, per-agent session quotas, explicit escalation tools, sensitive over-collection, and cross-tool record piggybacks are decided with no model and no network. Quota exceed is `deny` (never approve, never silent drop). Escalation tools return `approve` / `pending_approval`, never allow. Quota counts are file-backed (`out/quotas.json`) so a demo restart keeps the window.
